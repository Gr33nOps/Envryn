...
`audits/SECURITY_REMEDIATION_REPORT.md` §13/§14), so there is no `tauri-plugin-updater`, no
...
2. **An unsigned auto-updater is arguably worse than no updater.** Tauri's updater plugin signs
   ...
   `releasing/RELEASE_SIGNING.md`). Shipping an auto-updater before the underlying binaries are even signed
...
**Revisit this, not before:** real Authenticode signing exists (`releasing/RELEASE_SIGNING.md`), and there
...
anything reviewed in this document or in `releasing/RELEASE_SIGNING.md`.
...
2. **Verify the SHA-256 checksum before running anything.** Every release's notes publish the
   ...
   `audits/SECURITY_REMEDIATION_REPORT.md` and this pass's own build verification already record them.
   On Windows:
   ```powershell
...
3. **Run the new installer directly over the existing install** - this is a real in-place
   ...
   - The WiX (MSI) upgrade code is deterministically generated from the product name (see
     `releasing/RELEASE_SIGNING.md` §4's review of `WixConfig`) and stays stable release to release unless
   ...
   - Neither installer touches the vault database, which lives under the OS app-data directory
     ...
     (`Program Files` for MSI, a per-user directory for NSIS - see `releasing/RELEASE_SIGNING.md`'s sibling
     review and `releasing/CLEAN_VM_TEST_CHECKLIST.md` §1/§8 for where exactly). An in-place upgrade does
     not touch, migrate, or risk the vault.
4. **Once signing exists** (`releasing/RELEASE_SIGNING.md`), this same manual process gains a second,
...
- **Downgrade protection.** `WindowsConfig.allow_downgrades` defaults to `true` in Tauri's own
  schema (verified against the pinned `tauri-utils` source, see `releasing/RELEASE_SIGNING.md`), meaning a
