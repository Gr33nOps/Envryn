...
| **V-15** | Vault left unlocked and unattended | **Implemented:** system-wide idle poll (`GetLastInputInfo`, every 5s, configurable threshold) locks automatically, plus a direct `WTS_SESSION_LOCK` hook (`platform::windows_impl::watch_session_lock`, a window-procedure subclass) that fires the identical lock sequence the instant Windows reports the session locked — see `ARCHITECTURE.md` section 7. | Neither trigger exists on Android yet (spec's Android platform work is separately scoped, `ARCHITECTURE.md` section 7's table). The window-message hook can fail to register (best-effort), in which case coverage falls back to idle-poll-only, same as before this pass. |
