...
**Auditor:** Claude Code, acting on the user's explicit instruction to perform a full
production-readiness, security, privacy, code-quality, and runtime audit, fix real findings,
and re-verify.
...
incompatibility that isn't resolvable from inside a Claude Code tool-execution context.

**Concrete next step, not yet tried**: run `node .dev-tools/webdriver-smoke.mjs` from an actual
interactive desktop terminal (not through this tool) — if it works there, the cause is specific
to how Claude Code's Bash tool executes child processes on this machine, not WebView2/Edge
