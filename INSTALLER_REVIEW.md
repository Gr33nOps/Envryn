...
- The folder gets created within ~5 seconds of *any* launch, vault or no vault. This is expected,
  deliberate behavior, not a bug — auto-lock's whole design requires settings to be readable
  before a vault exists.
- **Nothing security-sensitive is created by this path.** `settings::load` only *reads*; only
  ...
  grepped for every `settingsSet` call site to confirm this is the only one) ever *writes*
...
- Docs and the clean-VM checklist that implied this folder only appears *after* vault creation
